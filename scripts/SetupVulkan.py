...
class VulkanConfiguration:
    ...
    def __InstallVulkanSDK(cls):
        ...
        while not permissionGranted:
            reply = str(input("Would you like to install VulkanSDK {0:s}? [Y/N]: ".format(cls.intallVulcanVersion))).lower().strip()[:1]
            if reply == 'n':
                return
            permissionGranted = (reply == 'y')

        vulkanInstallURL = f"https://sdk.lunarg.com/sdk/download/{cls.intallVulcanVersion}/windows/VulkanSDK-{cls.intallVulcanVersion}-Installer.exe"
        vulkanPath = f"{cls.vulkanDirectory}/VulkanSDK-{cls.intallVulcanVersion}-Installer.exe"

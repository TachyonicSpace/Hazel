...
namespace Hazel
{
	...
	std::vector<nc::NdArray<float>> NeuralNetwork<float>::forward(nc::NdArray<float> a1)
	{
		...
		std::vector<nc::NdArray<float>> results = { a1 };
		for (int i = 0; i < m_Weights.size(); i++)
		{
			results.emplace_back(nc::dot(m_Weights[i], results[i]) + m_biases[i]);
			//a1 = sigmoid(nc::dot(m_Weights[i], a1) + m_biases[i]);
		}
